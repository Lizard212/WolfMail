...
from core.exceptions import wolfException
import re
import sys
import os
import random
    ...
    def parse_mail_log(self, mail_type, mail_log, real_mail_name, mail_to):
        ...
        for line in log_file:
            if re.search(mail_reg, line):
                ...
                break
        return_posttfix="%s: to=<%s>.*status=(.*) %s" % (mail_id,
                                              mail_to, timestamp)
        return_sendmail="%s: to=<%s>.*stat=(.*) %s" % (mail_id,
                                            mail_to, timestamp)
        result_list = {"postfix":return_posttfix, "sendmail":return_sendmail}

        log_file.close()

        return result_list[mail_type]

    def ramdom_email(self):
        """
    ...
    def main(self, options, from_mail, from_mail_gecos, mail_to, subject, smtp_server, domain, html_file, verbose, mail_type, mail_log, attach=None):
        ...
        if options == "attach":
            ...
            try:
                ...
                if verbose:
                    print >> sys.stderr, bcolors.OKG + "[+] " + bcolors.ENDC + bcolors.FAIL + "%s"%(from_mail) + bcolors.ENDC + bcolors.FAIL + " -> " + bcolors.ENDC + bcolors.OK + "%s" %(mail_to) + bcolors.ENDC
                server.quit()
            except Exception, mess:
